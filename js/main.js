...
function addEventListener(event){
	...
	const item= button.closed('item')
	const itemTittle= item.querySelector('.item-tittle').textContent;
	const itemImagen= item.querySelector('item-image').src;
	const itemPrice= item.querySelector('item-price').textContent;

	addItemToShoppingCart(itemTittle,itemPrice,itemImagen);

}



function addItemToShoppingCart(itemTittle,itemPrice,itemImagen){
	shoppingCartItemsContainer.getElementsByClassName('shoppingCartItemTittle');
	for(let i=0;i<elementTittle.length;i++){
		if(elementTittle[i].innerText===itemTittle){
			let elementQuantity=elementTittle[i].parentElement.parentElement.parentElement.querySelector('.shoppingCartItemQuantity');
...
shoopingCartRow.querySelector('.buttonDelete').addEventListener ('click'.removeShoppingCartItem);

shoppingCartRow.querySelector('.shoppingCartItemQuantity').addEventListener('change',quantityChanged);

function updateShoppingCartTotal(){
	let total=0;
	const updateShoppingCartTotal= document.querySelector('.shoppingCartTotal');
	...
	shoppingCartItems.forEach(shoppingCartItem =>{
		...
		const shoppingCartItemQuantityElement= shoopinCartItem.querySelector('.shoppingCartItemQuantity');

		const shoppingCartItemQuantity = Number(shoppingCartItemQuantityElement.value);

		total=total+shoppingCartItemPrice*shoppingCartItemQuantity;
	})
	updateShoppingCartTotal.innerHTML=`${total.toFixed(0)}$`
}
